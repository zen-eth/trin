...
elif [ "$PORTAL_CLIENT" == "shisui" ]; then
    if [ "$SHISUI_PATH" == "" ]; then
      echo "env variable of SHISUI_PATH must be set";
      exit 1;
    fi
    # Run shisui sender
    run_shisui "$LOG_DIR/$DATA_DIR_SENDER" "$PORT_SENDER" "$EXT_PORT_SENDER" "$LOG_DIR/$DATA_DIR_SENDER"

    # Run shisui receiver
    run_shisui "$LOG_DIR/$DATA_DIR_RECEIVER" "$PORT_RECEIVER" "$EXT_PORT_RECEIVER" "$LOG_DIR/$DATA_DIR_RECEIVER"
fi
